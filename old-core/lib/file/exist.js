...
var fullpath = FILE_PATH.fullpath;
...
function tracePath(relativePath, fromPath, isList) {
    var parents = FILE_PATH.parentPaths(fromPath);
    var path = PATH;
    var c = 0;
    ...
    for (; length--; c++) {
        fullPath = path.join(parents[c], relativePath);

        if (pathIs(fullPath, isList)) {
            return fullPath;
        }
    }
